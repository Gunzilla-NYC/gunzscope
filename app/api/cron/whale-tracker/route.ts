...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const sub of subscribers) {
      ...
      if (emailSent) {
        pendingLogs.push({ userId: sub.userId, subject, meta: { dedupKey, activityCount: activities.length } });
        sent++;
      }
    }
    ...
    if (pendingLogs.length > 0) {
      after(() =>
        Promise.all(pendingLogs.map(l => logAlert(l.userId, 'whale_tracker', l.subject, l.meta)))
          .catch(e => console.error('[Cron:whale-tracker] logAlert error:', e))
      );
    }
