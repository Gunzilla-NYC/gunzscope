...
export default function WalletIdentity({
  ...
  const [switcherPosition, setSwitcherPosition] = useState<PopoverPosition>({ top: 0, left: 0 });
  ...
  const mode: DisplayMode = useMemo(() => {
    ...
    if (isAuthenticated && !isOwnWallet && primaryWalletAddress) return 'viewing';
    return 'simple';
  }, [isAuthenticated, isOwnWallet, hasPortfolioAddresses, primaryWalletAddress]);
  ...
  // Switcher popover position
  useLayoutEffect(() => {
    if (!showSwitcher) return;
    setSwitcherPosition(calcPosition(switcherTriggerRef.current, 300, 260));

    let scrollEnabled = false;
    const enableScrollClose = setTimeout(() => { scrollEnabled = true; }, 150);
    const handleScroll = () => { if (scrollEnabled) setShowSwitcher(false); };
    const handleResize = () => setSwitcherPosition(calcPosition(switcherTriggerRef.current, 300, 260));
  ...
  // Close on click outside (switcher)
  useEffect(() => {
    if (!showSwitcher) return;
    const handler = (e: MouseEvent) => {
      const t = e.target as Node;
      if (switcherRef.current && !switcherRef.current.contains(t) && switcherTriggerRef.current && !switcherTriggerRef.current.contains(t)) {
  ...
  // Close on escape (either popover)
  useEffect(() => {
    if (!showDetails && !showSwitcher) return;
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        if (showDetails) { setShowDetails(false); triggerRef.current?.focus(); }
        if (showSwitcher) { setShowSwitcher(false); switcherTriggerRef.current?.focus(); }
      }
    };
    document.addEventListener('keydown', handler);
    return () => document.removeEventListener('keydown', handler);
  }, [showDetails, showSwitcher]);
  ...
  useEffect(() => {
    if (showSwitcher && switcherRef.current) {
      const t = setTimeout(() => switcherRef.current?.focus({ preventScroll: true }), 10);
      return () => clearTimeout(t);
    }
  }, [showSwitcher]);
  ...
  // --- Popover: Wallet Switcher ---
  const switcherPopover = (
    <div
      ref={switcherRef}
      className="fixed w-[300px] bg-[#0a0a0a] border border-white/[0.06] shadow-2xl shadow-black/80 overflow-hidden"
      style={{
        top: switcherPosition.top,
        left: switcherPosition.left,
        zIndex: 9999,
        clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))',
      }}
      role="dialog"
    ...
    >
      ...
      <div className="px-4 py-2.5 border-b border-white/[0.06] flex items-center justify-between">
        <span className="font-mono text-[9px] tracking-[0.15em] uppercase text-[var(--gs-gray-3)]">
          Portfolio Wallets
        </span>
        <span className="font-mono text-[9px] tracking-[0.15em] uppercase text-[var(--gs-purple)]">
      ...
      <div className="py-1 max-h-[240px] overflow-y-auto">
        ...
        {/* Portfolio addresses (excluding primary) */}
        {portfolioAddresses
      ...
      <div className="px-4 py-2.5 border-t border-white/[0.06]">
        <Link
          ...
          className="flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-wider text-[var(--gs-gray-3)] hover:text-[var(--gs-lime)] transition-colors"
        >
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="flex items-center gap-3">
        ...
        {/* Switcher button (only in switcher mode) */}
        {mode === 'switcher' && (
          <button
            ref={switcherTriggerRef}
            onClick={toggleSwitcher}
            className="flex items-center gap-1 text-[11px] text-[var(--gs-gray-3)] hover:text-[var(--gs-lime)] hover:bg-white/5 px-2 py-1 rounded transition font-mono uppercase tracking-wider cursor-pointer"
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
      ...
      {mounted && showSwitcher && createPortal(switcherPopover, document.body)}
    </div>
  );
}
